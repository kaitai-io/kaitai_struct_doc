...
The YAML-based syntax might look a little more verbose than C-like struct,
...
main header into separate subtype, so we'll have to access it using the `.`
...
many formats. The basic idea is that we do a modular and
...
We've introduced the `doc` key <<docstrings,early in this user guide>> as
