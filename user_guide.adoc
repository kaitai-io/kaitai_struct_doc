...
level - i.e. the same `seq` designation. Note that there's no need for
`meta` here, as type name is derive from `types` key name here.
...
Struct. In a nutshell, it is a simple object-oriented language that
gets translated/compiled (AKA "transpiled") into any supported target
...
literals), and it's possible to call methods on it.
